...
class FAN(object):
    ...
    """
    def __init__(self, device='cpu', target_size=224, face_detector='sfd', lm_type='2D',
                 use_prev_fan_bbox=False, use_prev_bbox=False):

        self.model = FaceAlignment(getattr(LandmarksType, f'_{lm_type}'), flip_input=False,
    ...
    def __call__(self, image=None):
        ...
        if image is not None:
            ...
            if self.img_orig is None:
                raise ValueError("If no `image` is supplied, `self.img_orig` should exist!")
        ...
        lm, _, fan_bbox = self.model.get_landmarks_from_image(
            self.img_orig,
            detected_faces=prev_fan_bbox,
            return_bboxes=True
        )
        
        if lm is None and self.prev_fan_bbox is not None:
            # Second try: without previous FAN bbox
            lm, _, fan_bbox = self.model.get_landmarks_from_image(self.img_orig, return_bboxes=True)
    ...
    def _preprocess(self):
        ...
        img = resize(self.img_crop, (self.target_size, self.target_size), anti_aliasing=True)        
