...
from ..utils import get_logger, rescale
...
class Base4D:
    ...
    def __init__(self, v, f, mat=None, cam_mat=None, frame_t=None, events=None, sf=None,
    ...
    def _check(self):
        ...
        if self.frame_t.size > T:
            self.logger.warn(f"More frame times {self.frame_t.size} than vertex "
                              "time points ({T}); trimming ...")
            self.frame_t = self.frame_t[:T]

        if self.mat is not None:
            if self.mat.shape[0] != T:
                mT = self.mat.shape[0]
                self.logger.warn(f"More mats ({mT}) than vertex time points ({T}); "
                                 "trimming ...")
                self.mat = self.mat[:T, :, :]
    ...
    def save(self, path, compression_level=9):
        ...
        # because Pandas assumes a buffer or path, not an opened h5py file
        if self.events is not None:
    ...
    def load(path):
        ...
        with h5py.File(path, "r") as f_in:
            ...
            if 'loglevel' not in f_in.attrs:
                init_kwargs['loglevel'] = 20
            else:
                init_kwargs['loglevel'] = f_in.attrs['loglevel']
    ...
    def to_mne_rawarray(self):
        ...
        >>> from medusa.data import get_example_h5  # doctest: +SKIP
        >>> data = get_example_h5(load=True)        # doctest: +SKIP
        >>> rawarray = data.to_mne_rawarray()       # doctest: +SKIP
        """

        try:
            import mne
        except ImportError:
            print("Package 'mne' not installed!")
            return None
    ...
    def render_video(self, f_out, renderer, video=None, scale=None, n_frames=None,
        ...
        writer = imageio.get_writer(f_out, mode="I", fps=float(self.sf))
    ...
    def plot_data(self, f_out, plot_motion=True, plot_pca=True, n_pca=3):
        ...
        if plot_motion:
            motion = self.decompose_mats(to_df=True)
...
class Fan4D(Base4D):
    ...
    **kwargs: dict
    ...
    def load(cls, path):
        ...
        >>> from medusa.data import get_example_video
        >>> path = get_example_video()
        >>> fan_data = videorecon(path, recon_model='fan', device='cpu')
        """

        init_kwargs = super().load(path)
        return cls(**init_kwargs)

    def render_video(self, f_out, video=None, scaling=None, n_frames=None):
        """ Renders a video of the reconstructed vertices. 
