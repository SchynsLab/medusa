...
class EpochsArray(mne.epochs.EpochsArray):
    ...
    @classmethod
    def from_gmfx(cls, v, sf, events=None, frame_t=None, tmin=-.5, includes_motion=False):
        ...
        return cls(v, info, tmin=tmin, events=events_, event_id=event_id,
                   verbose='WARNING')
    ...
    def events_to_mne(events, frame_t):
        ...
        for i, (_, ev) in enumerate(events.iterrows()):
            ...
            if np.min(t_diff) > 0.05:
                ...
                raise ValueError(f"Nearest sample is {min_} seconds away "
