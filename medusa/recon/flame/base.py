...
class FlameReconModel(BaseReconModel):
    ...
    def get_tris(self):
        ...
        if not hasattr(self, '_tris'):
            ...
            self._tris = torch.as_tensor(template['f'], device=self.device, dtype=torch.int64)
        ...
        if hasattr(self, '_active_mask') and self._tris.shape[0] == 9976:
            ...
            self._tris = torch.as_tensor([lut[x.item()] for x in self._tris[idx].flatten()],
    ...
    def apply_mask(self, name, v):
        ...
        if name in self._masks:
            self._active_mask = torch.as_tensor(self._masks[name], dtype=torch.int64, device=self.device)
        else:
