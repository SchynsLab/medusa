...
.. py:class:: VideoLoader(path, batch_size=32, channels_first=False, device=DEVICE, **kwargs)
...
.. py:class:: VideoDataset(video, device=DEVICE)
...
.. py:function:: load_obj(f, device=None)
   ...
   Loads data from obj file, based on the DECA implementation, which in turn is based
   on the pytorch3d implementation.
