...
class Demo:
    ...
    def __init__(self, vid, recon, f_out, scale=1, background_color=(0, 0, 0), loglevel='INFO'):
    ...
    def start(self):
        ...
        self.logger.info(f"Opening {self.f_out} with size {vid_size} and "
                         f"{self.fps:.2f} FPS")
        self._vid_size = vid_size

        # Setup renderer
        self._renderer = Renderer(viewport=vid_size, camera_type='intrinsic',
                                  wireframe=True)
    ...
    def _blend(self, bg, img, alpha=None):
        ...
        if alpha is None:
            alpha = np.array(img.getchannel('A')) / 255.
            alpha = alpha[..., None]
    ...
    def play(self, duration):
        ...
        for i_frame in pbar:
            ...
            if self._should_show(t, self.recon_params):
                ...
                recon_img = self._renderer(recon.v[i_frame + self.current_frame, ...], recon.f)
            ...
            for txt_params in self.txt_params:
                if self._should_show(t, txt_params):
                    ...
                    params = {k: v for k, v in txt_params.items() if k not in ['start', 'end', 'fade_in', 'fade_out']}
                    draw.text(**params)
    ...
    def _resize(self, img, to_pil=True):
        img = rescale(img, self.scale, anti_aliasing=True, channel_axis=2,
...
if __name__ == '__main__':
    ...
    demo.show_txt(start=0, end=1.5, text=intro, pos=(0.5, 0.5), align='center', anchor='ms', fontsize=15)
    demo.show_vid(start=1.5, end=length, fade_in=1, fade_out=None)
    demo.show_txt(start=2, end=3, text='Medusa can', pos=(0.05, 0.05), fontsize=25, fade_in=0.5, fade_out=0.5)
    ...
    demo.show_txt(start=4.5, end=6.5, text='and analyze\n\nface images and videos', pos=(0.05, 0.4), fontsize=20)

    demo.show_txt(start=6, end=7, text='And supports reconstruction models like', pos=(0.05, 0.05), fontsize=20, fade_in=0.5)
    demo.show_txt(start=7, end=8, text='And supports reconstruction models like', pos=(0.05, 0.05), fontsize=20)
